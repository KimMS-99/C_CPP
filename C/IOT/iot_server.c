...
{
	char pass_check[ID_SIZE], id_check[ID_SIZE];
	...
	FILE *f;
	f = fopen("idpasswd.txt", "r");
	CLIENT_INFO *client_info = (CLIENT_INFO *)malloc(sizeof(CLIENT_INFO) * MAX_CLNT);

	for (i = 0; i < MAX_CLNT; i++)
	{
		client_info[i].index = 0;
		client_info[i].fd = -1;
		fscanf(f, "%s %s", id_check, pass_check);
		strcpy(client_info[i].id, id_check);
		strcpy(client_info[i].pw, pass_check);
	}
		...
		// 		{0,-1,"","HM_CON","PASSWD"}}; // 하드코딩된 id, passwd를 db나 file로 
